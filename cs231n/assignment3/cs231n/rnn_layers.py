...
def rnn_step_backward(dnext_h, cache):
    ...
    dact = dnext_h * (1 - np.square(next_h))
    dx = np.dot(dact, Wx.T)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        x_t = x[:, t, :].reshape(N, D)
        h_t, c = rnn_step_forward(x_t, prev_h, Wx, Wh, b)
        h[:, t, :] = h_t
        prev_h = h_t
        cache.append(c)
    ##############################################################################
...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    dprev_h = np.zeros(dh0.shape)
    for t in range(T)[::-1]:
        dx[:, t, :], dprev_h, tmp_dWx, tmp_dWh, tmp_db = rnn_step_backward(dh[:, t, :] + dprev_h, cache[t])
        dWx += tmp_dWx
        dWh += tmp_dWh
        db  += tmp_db
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (i, f, o, g, tanh_c, prev_c, x, prev_h, Wx, Wh)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    i, f, o, g, tanh_c, prev_c, x, prev_h, Wx, Wh = cache
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    c_next = np.zeros((N, H))
    for t in range(T):
        h_next, c_next, cache_t = lstm_step_forward(x[:, t, :], h_next, c_next, Wx, Wh, b)
        h[:, t, :] = h_next
...
def lstm_backward(dh, cache):
    ...
    for t in range(T)[::-1]:
        dx[:, t, :], dprev_h_t, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dprev_h[:, t, :] + dprev_h_t, dprev_c, cache[t])
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
